...
/**
 * Create the JesterKit EXE adapter
...
const adapter = (options?: AdapterOptions): Adapter => {
	return {
		...
		adapt: async (builder) => {
			...
			builder.log.success("[JesterKit] Manifest generated");
			...
			builder.log.success("[JesterKit] Assets generated");

			// Step 6: Compile the app
			const { binaryPath, sizeInMb } = await compileApplication(builder, adapterOptions);
			builder.log.success(`[JesterKit] Application compiled (${sizeInMb} MB)`);
			...
			builder.log.success(`[JesterKit] Start the application with: ./${binaryPath}`);
		},
		supports: {
			read: () => true,
		},
