...
export async function bundle(external: string[] = []) {
	try {
		...
		if (error instanceof Error && error.message.includes("Could not resolve")) {
			...
			console.error('[JesterKit] "postbuild": "exe-tanstack --external=missing-package,@other/package"');
			console.error("[JesterKit] Or use the --external flag:");
			console.error("[JesterKit] exe-tanstack --external missing-package --external @other/package\n");
		}
		throw error;
	}
}
